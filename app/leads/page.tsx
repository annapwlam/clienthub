...
function LeadCard({ lead }: { lead: Lead }) {
  const overdue =
    ...
    ["new", "qualified", "proposal"].includes(lead.stage) &&
  ...
  return (
    ...
    >
      ...
      {overdue && (
        <p className="mt-2 inline-flex items-center gap-1 rounded bg-rose-50 px-1.5 py-0.5 text-[11px] font-medium text-rose-600">
          ⏰ Follow-up overdue
        </p>
      )}
    </Link>
  ...
  if (error) {
    return (
      <div className="rounded-xl border border-rose-200 bg-rose-50 p-8 text-center">
        <p className="font-semibold text-rose-700">Couldn&apos;t load the pipeline.</p>
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500">
            {total} lead{total === 1 ? "" : "s"} across {STAGES.length} stages
          </p>
        </div>
      ...
      {total === 0 ? (
        <div className="rounded-xl border-2 border-dashed border-slate-300 bg-white p-12 text-center">
          ...
          <h2 className="mt-2 font-semibold text-slate-900">No leads yet</h2>
          <p className="mx-auto mt-1 max-w-sm text-sm text-slate-500">
            Your pipeline is empty. Add your first lead to start tracking
